...
// 文件：nnls.cc
// 实现了 Lawson-Hanson NNLS 算法
// 从 nnls.c 复制过来，所以我没有版权
// ...其他人有版权......不知道这是否应该根据 GPL 或 LGPL 等许可证发布，
// 但我还是在这里写个小注释：

// 版权所有 (C) 2004 Lawson-Hanson
// 由 Suvrit Sra 修改以适应 C++。

// 本程序是自由软件；你可以在 GNU 通用公共许可证的条款下重新分发和/或
// 修改它；该许可证由自由软件基金会发布；可以是版本 2
// 的许可证，或者（根据你的选择）任何更高版本。

// 本程序的分发希望对你有用，
// 但不提供任何担保；甚至不包括对适销性或适合特定目的的隐含担保。详情见
// GNU 通用公共许可证。

// 你应该已收到 GNU 通用公共许可证的副本
// 随着本程序一起；如果没有，请写信给自由软件基金会，地址是
